...
func TestRedundantParenthesesFixes(t *testing.T) {
	path := filepath.Join(fixtureRoot(t), "redundant-parentheses", "invalid.pwn")
	source, err := os.ReadFile(path)
	if err != nil {
		t.Fatal(err)
	}
	diagnostics := ruletest.RunRule(t, path, "redundant-parentheses", source)
	...
	if remaining := ruletest.RunRule(t, path, "redundant-parentheses", updated); len(remaining) != 0 {
